...
create table varustus (
    varustusid int identity(1,1) primary key,
    nimetus varchar(50) not null,
    kirjeldus varchar(100),
    politseinik_politseinikid int not null
);

alter table varustus
add constraint fk_varustus_politseinik
foreign key (politseinik_politseinikid) references politseinik(politseinikid);
...
INSERT INTO Töögraafik (algus_kuupaev, lopp_kuupaev, Politseinik_politseinikID) VALUES
  ...
  insert into varustus (nimetus, kirjeldus, politseinik_politseinikid) values
  ('Pistol', 'ohuslik asi', 1),
  ('Käerauad', 'kasulik asi', 2),
  ('Kuulikindel vest', 'kui ei taha suri', 3),
  ('Raadio', 'komunikatsioon asi', 4);
